...
def handle_question(
    ...
    messages, source_snippets = prepare_chat_request(question, difficulty, mode)
...
def build_recent_topics() -> list[str]:
    ...
    for message in reversed(st.session_state.messages_for_display):
        if message["role"] == "user" and message.get("track_as_topic") and message["content"] not in topics:
